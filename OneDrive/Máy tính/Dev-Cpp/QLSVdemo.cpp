...
if (file.is_open()) {
    std::string line;
    while (std::getline(file, line) && n < 100) {
        // Tách các thông tin c?a sinh viên t? chu?i line
        std::istringstream iss(line);
        std::string maSV, tenSV, lopSV, gioitinhSV, nghanhhocSV;
        int namsinh;
        float diem1, diem2, diem3;

        iss >> maSV >> tenSV >> lopSV >> gioitinhSV >> nghanhhocSV >> namsinh >> diem1 >> diem2 >> diem3;

        // Tính di?m trung bình và luu thông tin vào m?ng dsSV
        double diemTB = (diem1 + diem2 + diem3) / 3.0;
        dsSV[n] = { maSV, tenSV, lopSV, gioitinhSV, nghanhhocSV, namsinh, diem1, diem2, diem3, diemTB };
        n++;
    }
    file.close();
}*/
